...
function Popup44(data) {
    ...
    mywindow.document.write('<html><head><title></title>');
    mywindow.document.write('</head><body >');
    mywindow.document.write($('#dvprint44').html());
    mywindow.document.write('</body></html>');
...
function loadData44() {
    var loadposturl = $('#loaddata44').val();
    $.ajax({
        url: loadposturl,
        type: "GET",
        contentType: "application/json;charset=utf-8",
...
function SetUpGrid44() {
    ...
    //check if datatable is already created then destroy iy and then create it
    if ($.fn.dataTable.isDataTable('#certtable_print4')) {
        table = $('#certtable_print4').DataTable();
        table.destroy();
    }

    // alert('hh');
    var table = $("#certtable_print4").DataTable({
        "dom": 'Bfrtip',
        "rowReorder": false,
        "ordering": false,
