...
/****** Object:  StoredProcedure [dbo].[GetIMONumber]    Script Date: 15-03-2021 05:07:57 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
Create procedure [dbo].[GetIMONumber]
as
begin
	select IMONumber
	from Ship
...
Select Id
		...
		,LatestUpdate
		,Comments
		,ISNULL(isApproved,0) AS isApproved from WorkSessions 
		--Select * from tblRegime FOR XML AUTO
End
...
AND CONVERT(VARCHAR(10),CreatedOn,102) = CONVERT(VARCHAR(10),@CreatedOn,102)   ---------- deep 13.02.2021
AND CONVERT(VARCHAR(10),LatestUpdate,102) = CONVERT(VARCHAR(10),@LatestUpdate,102)   ---------- deep 13.02.2021
...
-------------------------New
@IMAPPOP varchar(100),
@IMAPPOPServer varchar(100),
@IMAPPOPPort varchar(100)
--------------------------
...
Update tblConfig Set IsActive = 0 Where KeyName = 'imappopServer'
Insert into tblConfig (KeyName,ConfigValue,IsActive) Values ('imappopServer',@IMAPPOPServer,1)
Update tblConfig Set IsActive = 0 Where KeyName = 'imappopport'
Insert into tblConfig (KeyName,ConfigValue,IsActive) Values ('imappopport',@IMAPPOPPort,1)

Update tblConfig Set IsActive = 0 Where KeyName = 'protocol'
Insert into tblConfig (KeyName,ConfigValue,IsActive) Values ('protocol',@IMAPPOP,1)
...
SET @EmployeeNum = CAST(@MAXCREWVAL AS VARCHAR(4)) + '/' + CAST(@YearValue AS VARCHAR(4))
INSERT INTO Crew 
           (FirstName,MiddleName,LastName,Gender,RankID,CreatedOn,LatestUpdate,CountryID,DOB,POB,PassportSeamanPassportBook,Seaman,Notes,Watchkeeper,OvertimeEnabled,EmployeeNumber,VesselID,DepartmentMasterID,DeactivationDate,DeletionDate, IssuingStateOfIdentityDocument, ExpiryDateOfIdentityDocument, IsActive)  
Values(@FirstName,@MiddleName,@LastName,@Gender,@RankID,@CreatedOn,@ActiveTo,@CountryID,@DOB,@POB,@PassportSeamanPassportBook,@Seaman,@Notes,@Watchkeeper,@OvertimeEnabled,@EmployeeNum,@VesselID,@DepartmentMasterID,'12/31/9999','12/31/9999', @IssuingStateOfIdentityDocument, @ExpiryDateOfIdentityDocument, 1)  
DECLARE @CrewId int
SET @CrewId = @@IDENTITY
...
UPDATE Crew 
  SET IsActive= 0,
	  ...
	  LatestUpdate=GETDATE(),      ---------------------------------------------------------- new deep
	  DeletionDate = DATEADD (dd, -1, DATEADD(mm, DATEDIFF(mm, 0, GETDATE()) + 1, 0))
WHERE ID= @ID 

Update ServiceTerms Set ActiveTo=GETDATE()    ----------------------------------------------- new deep
WHERE ID= @ID                                 ----------------------------------------------- new deep
...
/****** Object:  StoredProcedure [dbo].[stpGetDaysLeft]    Script Date: 23-03-2021 05:19:57 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
-- exec stpGetDaysLeft 12
CREATE procedure [dbo].[stpGetDaysLeft]
(
@ID int
)
...
Select
Case 
When DATEDIFF(Day, Getdate(), LatestUpdate) > 3 Then 0
When DATEDIFF(Day, Getdate(), LatestUpdate) <= 3 Then 1
Else 10
End
As DaysLeft
from Crew where ID = @ID
End
GO
...
INSERT [dbo].[tblConfig] ([KeyName], [ConfigValue], [IsActive]) VALUES (N'rhsubject', N'RHDATASYNC', 1)

INSERT [dbo].[tblConfig] ([KeyName], [ConfigValue], [IsActive]) VALUES (N'smtp', N'xxx', 0)
INSERT [dbo].[tblConfig] ([KeyName], [ConfigValue], [IsActive]) VALUES (N'port', N'xxx', 0)
INSERT [dbo].[tblConfig] ([KeyName], [ConfigValue], [IsActive]) VALUES (N'shipemail', N'xxx', 0)
INSERT [dbo].[tblConfig] ([KeyName], [ConfigValue], [IsActive]) VALUES (N'shipemailpwd', N'xxx', 0)
INSERT [dbo].[tblConfig] ([KeyName], [ConfigValue], [IsActive]) VALUES (N'admincenteremail', N'xxx', 0)
INSERT [dbo].[tblConfig] ([KeyName], [ConfigValue], [IsActive]) VALUES (N'imappopServer', N'xxx', 0)
INSERT [dbo].[tblConfig] ([KeyName], [ConfigValue], [IsActive]) VALUES (N'imappopport', N'xxx', 0)
INSERT [dbo].[tblConfig] ([KeyName], [ConfigValue], [IsActive]) VALUES (N'protocol', N'xxx', 0)
